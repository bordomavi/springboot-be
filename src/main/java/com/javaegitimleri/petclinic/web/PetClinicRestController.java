...
public class PetClinicRestController {
	...
	public ResponseEntity<?> updateOwner(@PathVariable ("id") Long id,@RequestBody Owner requestOWner){
		...
		try {
			Owner owner = petClinicService.findOwner(id);
			owner.setFirstName(requestOWner.getFirstName());
			owner.setLastName(requestOWner.getLastName());
	...
	@RequestMapping(method=RequestMethod.GET,value="/owner/{id}",produces = "application/json") // produces /owner/{id} bundan birden fazla uri olsa bile 
	public ResponseEntity<?> getOwnerAsHateoasResource(@PathVariable ("id") Long id){           // json formatında bi istek alırsa direk buna düsecek
		try {
			Owner owner = petClinicService.findOwner(id);
